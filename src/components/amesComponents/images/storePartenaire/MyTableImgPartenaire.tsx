...
import { Images, useDeleteImage_partenaireimagesMutation, useGetAllImage_partenaireimagesAllQuery } from '../../../../features/api/storePartenaireApiSlice';
import Base64Image from '../../../../common/Base64Image';
import {setimgIdSlicePayload } from '../../../../features/reduxSlices/imgIdSlice';
import ModalImgPartenaireEdit from './ModalImgPartenaireEdit';


const MyTableImgPartenaire = () => {

  // Fetch data using the hook
  const { data, error, isLoading } = useGetAllImage_partenaireimagesAllQuery()
  const [deleteData] = useDeleteImage_partenaireimagesMutation() // Import the mutation hook
...
{data?.map((packageItem, key) => (
  ...
  <Base64Image base64String= {packageItem.imagedata} />
    ...
    <div className="mt-3 mb-3 flex items-center space-x-3.5">
      ...
      <ModalImgPartenaireEdit  isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} />
      </div>
    </div>
  );
};

export default MyTableImgPartenaire;
