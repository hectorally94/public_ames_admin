...
import ModalTypeImageImageStoreEdit from './ModalTypeImageImageStoreEdit';
import {Type_image_image_storeDto, useDeleteType_image_image_storeMutation, useGetAllType_image_image_storeQuery,  } from '../../../../features/api/storeTypeImageImageStore';


interface MyResponse {
  message: string;
  data: Type_image_image_storeDto[];
} 
const MyTableTypeImageImageStore = () => {

  // Fetch data using the hook
  const { data, error, isLoading } = useGetAllType_image_image_storeQuery()
  const [deleteData] = useDeleteType_image_image_storeMutation () // Import the mutation hook
    ...
    if (deleteId !== null) {
      ...
      try {
        ...
        setAlert({
          type: 'success',
          title: 'Success',
          message: 'TypeImage deleted successfully!',
        });
      } catch (err) {
        console.error('Failed to delete TypeImage:', err);
        setAlert({
          type: 'error',
          title: 'Error',
          message: 'Failed to delete TypeImage.',
        });
      } finally {
...
{packageData?.map((packageItem, key) => (
  ...
  <Base64Image base64String= {packageItem.imageStoreUrl} />

    <h5 className="text-xl mt-2 mb-2 font-bold tracking-tight text-gray-900 dark:text-white">
    ...
    <div className="mt-3 mb-3 flex items-center space-x-3.5">
      ...
      <ModalTypeImageImageStoreEdit  isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} />
      </div>
    </div>
  );
};

export default MyTableTypeImageImageStore;
