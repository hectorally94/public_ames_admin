...
import { useGetAllImage_store_imagesAllQuery } from "../../../../features/api/storeImageApiSlice";
...
import ModalAddTypeImageImageStore from "./ModalAddTypeImageImageStore";

interface ModalImage_profilesimageAddProps {
  isOpen: boolean;
  onClose: () => void;
}

const ModalTypeImageImageStore: React.FC<ModalImage_profilesimageAddProps> = ({ isOpen, onClose }) => {
  
  // Fetch data using the hook
  const { data, error, isLoading } = useGetAllImage_store_imagesAllQuery()
 ...
 useEffect(() => {
  ...
  return (
    <ModalImg title="Add type and Image" isOpen={isOpen} onClose={onClose}>
...
{(data || [])?.map((packageItem, key) => (
  ...
  <Base64Image base64String= {packageItem.imagedata} />
    ...
    <div className="mt-3 mb-3 flex items-center space-x-3.5">
      ...
      <ModalAddTypeImageImageStore  isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} />
      </div>
    </div>
...
export default ModalTypeImageImageStore;
