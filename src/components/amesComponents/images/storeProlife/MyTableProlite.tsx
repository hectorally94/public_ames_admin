...
import ModalProliteEdit from './ModalProliteEdit';
import { useDeleteImage_profilesimagesMutation, useGetAllImage_profilesimagesAllQuery } from '../../../../features/api/storeProfileApiSlice';

const MyTableProlite = () => {

  // Fetch data using the hook
  const { data, error, isLoading } = useGetAllImage_profilesimagesAllQuery()
  const [deleteData] = useDeleteImage_profilesimagesMutation() // Import the mutation hook
...
{data?.map((packageItem, key) => (
  ...
  <Base64Image base64String= {packageItem.imagedata} />
    ...
    <div className="mt-3 mb-3 flex items-center space-x-3.5">
      ...
      <ModalProliteEdit  isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} />
      </div>
    </div>
  );
};

export default MyTableProlite;
