...
import ModalTypeImageEdit from './ModalTypeImageEdit';
import { TypeImage, useDeleteTypeImageMutation, useGetAllTypeImagesQuery } from '../../../../features/api/typeImagesApiSlice';
...
const MyTableTypeImage = () => {

  // Fetch data using the hook
  const { data, error, isLoading } = useGetAllTypeImagesQuery();
  const [deleteTypeImage] = useDeleteTypeImageMutation(); // Import the mutation hook
 ...
 useEffect(() => {
  ...
  const confirmDelete = async () => {
    if (deleteId !== null) {
      ...
      try {
        await deleteTypeImage(deleteId).unwrap();
        setAlert({
          type: 'success',
          title: 'Success',
  ...
  >
    ...
    />
      ...
      <ModalTypeImageEdit  isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} />
      </div>
    </div>
  );
};

export default MyTableTypeImage;
