...
import ModalImagesEdit from './ModalImagesEdit';
import { useDeleteImage_store_imagesMutation, useGetAllImage_store_imagesAllQuery } from '../../../../features/api/storeImageApiSlice';

const MyTableImages = () => {

  // Fetch data using the hook
  const { data, error, isLoading } = useGetAllImage_store_imagesAllQuery()
  const [deleteData] = useDeleteImage_store_imagesMutation () // Import the mutation hook
...
{(data || [])?.map((packageItem, key) => (
  ...
  <Base64Image base64String= {packageItem.imagedata} />
    ...
    <div className="mt-3 mb-3 flex items-center space-x-3.5">
      ...
      <ModalImagesEdit  isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} />
      </div>
    </div>
  );
};

export default MyTableImages;
