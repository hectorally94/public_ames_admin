...
import { useDeleteVersionMutation, useGetAllVersionsQuery, Versions } from '../../../features/api/versionsApiSlice';
import ModalTypeVersionEdit from './ModalTypeVersionEdit';
...
const MyTableTypeVersion = () => {

  // Fetch data using the hook
  const { data, error, isLoading } = useGetAllVersionsQuery();
  const [deleteTypeVersion] = useDeleteVersionMutation(); // Import the mutation hook
 ...
 useEffect(() => {
  ...
  const confirmDelete = async () => {
    if (deleteId !== null) {
      ...
      try {
        await deleteTypeVersion(deleteId).unwrap();
        setAlert({
          type: 'success',
          title: 'Success',
  ...
  >
    ...
    />
      ...
      <ModalTypeVersionEdit  isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} />
      </div>
    </div>
  );
};

export default MyTableTypeVersion;
