...
import ModalPostAdd from "./ModalPostAdd";

interface ModalAddProps {
  isOpen: boolean;
  onClose: () => void;
}
const ModalLoadImageStore: React.FC<ModalAddProps> = ({ isOpen, onClose }) => {
...
{(data || [])?.map((packageItem, key) => (
  ...
  <Base64Image base64String= {packageItem.imagedata} />
    ...
    <div className="mt-3 mb-3 flex items-center space-x-3.5">
      ...
      <ModalPostAdd  isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} />
      </div>
    </div>
...
export default ModalLoadImageStore;
