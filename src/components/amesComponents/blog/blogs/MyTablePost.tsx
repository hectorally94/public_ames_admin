...
import { BlogDto, useDeleteBlogMutation, useGetPaginatedBlogQuery } from '../../../../features/api/blogsApiSlice';
import CardBlogComponent from '../../../../common/CardBlogComponent';
import { setSelectedBlog } from '../../../../features/reduxSlices/postBlogSlice';
...
/// pagination config
  ...
  const { data, error, isLoading } = useGetPaginatedBlogQuery({ title, page: currentPage, size: itemsPerPage, sort });
 
  const [deleteData] = useDeleteBlogMutation () // Import the mutation hook
 ...
 useEffect(() => {
  ...
  const handleEdit = (action:BlogDto) => {
    // Here, you can access the entire `selectedPackageItem` object
    dispatch(setSelectedBlog(action))
    ...
    <div key={key} className="relative flex bg-clip-border text-gray-700 shadow-md w-full flex-row">
      ...
      <CardBlogComponent
        ...
        recolte={packageItem.recolte}
         category={packageItem.categoryBlogTranslate}
