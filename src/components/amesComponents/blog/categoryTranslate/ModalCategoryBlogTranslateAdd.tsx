...
import { useSelector } from "react-redux";
 import { useCreateCategoryBlogTranslateMutation } from "../../../../features/api/categoryBlogTranslateApiSlice";

interface ModalTypeBlogAddProps {
  isOpen: boolean;
  onClose: () => void;
}

const ModalCategoryBlogTranslateAdd: React.FC<ModalTypeBlogAddProps> = ({ isOpen, onClose }) => {
  const selectedId = useSelector((state: RootState) => state.languageid.selectedId);
console.log("selectedId",selectedId)

  const [type, setType] = useState<string>("");
  const [CreateParticipable] = useCreateCategoryBlogTranslateMutation();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await CreateParticipable({languageId:selectedId, data:{id:0,categoryBlogTranslate: type} }).unwrap();
      setAlert({
          type: "success",
          title: "Success",
...
export default ModalCategoryBlogTranslateAdd;
