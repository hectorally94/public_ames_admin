...
import { useCreateActionMutation } from "../../../../features/api/actionsApiSlice";
import { TypeActionsTranslate, useGetAllTypeActionsTranslatesQuery } from "../../../../features/api/typeActionsTranslateApiSlice";
import { CategoryActionTranslate, useGetAllCategoryActionTranslatesQuery } from "../../../../features/api/categoryActionTranslateApiSlice";
...
const ModalPostAdd: React.FC<ModalAddTypeImageImageStoreProps> = ({ isOpen, onClose }) => {
  ...
  const [Create] = useCreateActionMutation();

  const { data: typeActionsData2} = useGetAllTypeActionsTranslatesQuery();
  const { data: categoryActionsData3 } = useGetAllCategoryActionTranslatesQuery();
  ...
  const filteredCategories = categoryActionsData.filter(category =>
    category?.categoryTranslate?.toLowerCase().includes(categoryFilter.toLowerCase())
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await Create({
        typeActionsTranslateId: selectedType,
