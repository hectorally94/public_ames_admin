// components/ModalTypeActionEdit.tsx
import React, { useEffect, useState } from "react";
import { useSelector } from "react-redux";
import { useUpdateTypeActionMutation } from "../../../../features/api/typeActionsApiSlice";
import Modal from "../../../modal/Modal";
import Alert from "../../AmesAlert/Alert";
import SmallLoader from "../../../../common/Loader/SmallLoader";
import { RootState } from "../../../../common/store";

interface ModalTypeActionEditProps {
  isOpen: boolean;
  onClose: () => void;
}

const ModalTypeActionEdit: React.FC<ModalTypeActionEditProps> = ({ isOpen, onClose }) => {
  const selectedTypeAction = useSelector((state: RootState) => state.typeAction.payload);
  const [type, setType] = useState(selectedTypeAction?.type || "");
  const [alert, setAlert] = useState<{ type: 'success' | 'warning' | 'error'; title: string; message: string } | null>(null);
  const [updateTypeAction] = useUpdateTypeActionMutation();
  const [loading, setLoading] = useState(false); // State to track loading

  useEffect(() => {
    if (selectedTypeAction) {
      setType(selectedTypeAction.type || "");
    }
  }, [selectedTypeAction]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await updateTypeAction({
        id: selectedTypeAction?.id!,
...
export default ModalTypeActionEdit;
