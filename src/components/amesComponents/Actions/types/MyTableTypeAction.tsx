...
import { TypeAction, useDeleteTypeActionMutation, useGetAllTypeActionsQuery } from '../../../../features/api/typeActionsApiSlice';
...
import ModalTypeActionEdit from './ModalTypeActionEdit';
import { setActionPayload } from '../../../../features/reduxSlices/typeActionSlice';
...
const MyTableTypeAction = () => {

  // Fetch data using the hook
  const { data, error, isLoading } = useGetAllTypeActionsQuery();
  const [deleteTypeAction] = useDeleteTypeActionMutation(); // Import the mutation hook
 ...
 useEffect(() => {
  ...
  const handleEdit = (selectedPackageItem:any) => {
    ...
    dispatch(setActionPayload(selectedPackageItem))
  ...
  const confirmDelete = async () => {
    if (deleteId !== null) {
      ...
      try {
        await deleteTypeAction(deleteId).unwrap();
        setAlert({
          type: 'success',
          title: 'Success',
  ...
  >
    ...
    />
      ...
      <ModalTypeActionEdit  isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} />
      </div>
    </div>
  );
};

export default MyTableTypeAction;
