...
import { useCreateCategoryEventTranslateMutation } from "../../../../features/api/categoryEventTranslateApiSlice";
...
const ModalCategoryActionTranslateAdd: React.FC<ModalTypeActionAddProps> = ({ isOpen, onClose }) => {
  const selectedId = useSelector((state: RootState) => state.languageid.selectedId);
console.log("selectedId",selectedId)

  const [type, setType] = useState<string>("");
  const [CreateParticipable] = useCreateCategoryEventTranslateMutation();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await CreateParticipable({languageId:selectedId, data:{id:0,categoryEventTranslate: type} }).unwrap();
      setAlert({
          type: "success",
          title: "Success",
...
export default ModalCategoryActionTranslateAdd;
