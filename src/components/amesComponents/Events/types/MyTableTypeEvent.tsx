...
import { TypeEvent, useDeleteTypeEventMutation, useGetAllTypeEventsQuery } from '../../../../features/api/typeEventsApiSlice';
import ModalTypeEventEdit from './ModalTypeEventEdit';
...
const MyTableTypeEvent = () => {

  // Fetch data using the hook
  const { data, error, isLoading } = useGetAllTypeEventsQuery();
  const [deleteTypeEvent] = useDeleteTypeEventMutation(); // Import the mutation hook
 ...
 useEffect(() => {
  ...
  const confirmDelete = async () => {
    if (deleteId !== null) {
      ...
      try {
        await deleteTypeEvent(deleteId).unwrap();
        setAlert({
          type: 'success',
          title: 'Success',
  ...
  >
    ...
    />
      ...
      <ModalTypeEventEdit  isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} />
      </div>
    </div>
  );
};

export default MyTableTypeEvent;
