...
import { useCreateTypeEventsTranslateMutation } from "../../../../features/api/typeEventsTranslateApiSlice";

interface ModalTypeEventAddProps {
  isOpen: boolean;
  onClose: () => void;
}

const ModalTypeEventTranslateAdd: React.FC<ModalTypeEventAddProps> = ({ isOpen, onClose }) => {
  const selectedId = useSelector((state: RootState) => state.languageid.selectedId);
console.log("selectedId",selectedId)

  const [type, setType] = useState<string>("");
  const [CreateParticipable] = useCreateTypeEventsTranslateMutation();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        await CreateParticipable({languageId:selectedId, data:{id:0,typeTranslate: type} }).unwrap();
        setAlert({
          type: "success",
          title: "Success",
  ...
  return (
    <Modal title="Add Participate text" isOpen={isOpen} onClose={onClose}>
      {alert && (
        <Alert
          type={alert.type}
          title={alert.title}
...
export default ModalTypeEventTranslateAdd;
