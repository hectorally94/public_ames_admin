...
import { ImpactDto, useDeleteImpactMutation, useGetAllImpactsDtoQuery } from '../../../features/api/impactsApiSlice';
import ModalImpactEdit from './ModalImpactEdit';
...
const MyTableImpact = () => {
  ...
  const { data, error, isLoading } = useGetAllImpactsDtoQuery()
  const [deleteData] = useDeleteImpactMutation(); // Import the mutation hook
 ...
 useEffect(() => {
  ...
  >
    ...
    />
      ...
      <ModalImpactEdit  isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} />
      </div>
    </div>
  );
};

export default MyTableImpact;
