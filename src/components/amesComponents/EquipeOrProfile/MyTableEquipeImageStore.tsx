...
import ModalEquipeImageStoreEdit from './ModalEquipeImageStoreEdit';
import { NotreEquipeDto, useDeleteNotreEquipeMutation, useGetAllNotreEquipeQuery } from '../../../features/api/notreEquipeApiSlice';
...
const MyTableEquipeImageStore = () => {

  // Fetch data using the hook
  const { data, error, isLoading } = useGetAllNotreEquipeQuery()
  const [deleteData] = useDeleteNotreEquipeMutation () // Import the mutation hook
...
{packageData?.map((packageItem, key) => (
  ...
  <Base64Image base64String= {packageItem.imagedata} />
    ...
    <div className="mt-3 mb-3 flex items-center space-x-3.5">
      ...
      <ModalEquipeImageStoreEdit  isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} />
      </div>
    </div>
  );
};

export default MyTableEquipeImageStore;
