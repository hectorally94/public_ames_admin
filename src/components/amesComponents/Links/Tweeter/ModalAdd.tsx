...
import Alert from "../../AmesAlert/Alert";
 ...
 import { useCreateLinkTweeterMutation } from "../../../../features/api/linkTweeterApiSlice";
...
const ModalAdd: React.FC<ModalAddProps> = ({ isOpen, onClose }) => {
  ...
  const [createLanguage] = useCreateLinkTweeterMutation(); // Hook for creating a language
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await createLanguage({ name }).unwrap(); // Unwrap to handle success or error directly
  ...
  return (
    <Modal title="Add Language" isOpen={isOpen} onClose={onClose}>
      {alert && (
        <Alert
          type={alert.type}
          title={alert.title}
        ...
        <form onSubmit={handleSubmit}>
          <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
            <div className="p-6.5">
              ...
              >
      {loading ? <SmallLoader /> : 'Add Language'}
      </button>
            </div>
          </div>
        </form>
