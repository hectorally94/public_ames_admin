...
import { Info, useDeleteInfoMutation, useGetAllInfoQuery } from '../../../features/api/infoApiSlice';
import ModalTypeInfoEdit from './ModalTypeInfoEdit';
...
const MyTableTypeInfo = () => {

  // Fetch data using the hook
  const { data, error, isLoading } = useGetAllInfoQuery();
  const [deleteTypeInfo] = useDeleteInfoMutation (); // Import the mutation hook
 ...
 useEffect(() => {
  ...
  const confirmDelete = async () => {
    if (deleteId !== null) {
      ...
      try {
        await deleteTypeInfo(deleteId).unwrap();
        setAlert({
          type: 'success',
          title: 'Success',
  ...
  >
    ...
    />
      ...
      {/* Modal for adding  */}
      <ModalTypeInfoEdit  isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} />
      </div>
    </div>
  );
};

export default MyTableTypeInfo;
