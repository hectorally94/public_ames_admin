...
import { useGetAllMissionsQuery, useDeleteMissionMutation, MissionDto } from '../../../features/api/missionsApiSlice';
import ModalMissionImageStoreEdit from './ModalMissionImageStoreEdit';
...
const MyTableMissionImageStore = () => {

  // Fetch data using the hook
  const { data, error, isLoading } = useGetAllMissionsQuery()
  const [deleteData] = useDeleteMissionMutation () // Import the mutation hook
...
{packageData?.map((packageItem, key) => (
  ...
  <Base64Image base64String= {packageItem.imageData} />
    ...
    <div className="mt-3 mb-3 flex items-center space-x-3.5">
      ...
      <ModalMissionImageStoreEdit  isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} />
      </div>
    </div>
  );
};

export default MyTableMissionImageStore;
