...
import ModalPartenaireImageStoreEdit from './ModalPartenaireImageStoreEdit';
import { PartenaireDto, useDeletePartenaireMutation, useGetAllPartenairesQuery } from '../../../features/api/partenairesApiSlice';
...
const MyTablePartenaireImageStore = () => {

  // Fetch data using the hook
  const { data, error, isLoading } = useGetAllPartenairesQuery()
  const [deleteData] = useDeletePartenaireMutation () // Import the mutation hook
...
{packageData?.map((packageItem, key) => (
  ...
  <Base64Image base64String= {packageItem.imagedata} />
    ...
    <div className="mt-3 mb-3 flex items-center space-x-3.5">
      ...
      <ModalPartenaireImageStoreEdit  isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} />
      </div>
    </div>
  );
};

export default MyTablePartenaireImageStore;
